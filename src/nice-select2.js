...
NiceSelect.prototype._sortBySimilarity = function (text) {
  if (text == "") {
    ...
    this.options = list.map((item, idx) => {
      if (idx < 10 && item.similarity > 0.5) {
        item.option.display = true;
        return item.option;
      } else {
        item.option.display = false;
        return item.option;
      }
    });
  }
